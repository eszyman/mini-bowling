...
// ==========================================
// 2. TURRET CONTROLLER FSM (CLOSED-LOOP DIRECT DRIVE)
// ==========================================
...
public:
    ...
    void begin() {
        ...
        pinMode(_hallPin, INPUT_PULLUP); pinMode(_relayPin, OUTPUT);
        ...
        switch (_state) {
            ...
            case WAITING_FOR_PIN:
                ...
                if (_queuedPins > 0) {
                    ...
                    _loadedCount++; 
                    if (_loadedCount == 9) {
                        conveyorOff(); // MUST halt immediately to prevent 10th pin fetch
                        tPrint("9 Pins Loaded. Hard Locking Turret.");
                        changeState(PAUSED_AT_9);
                    } else {
                        conveyorOn(); // Maintain belt pressure for 1-8
            ...
            case CATCH_DELAY_PHASE:
                conveyorOn(); // Belt stays on while gravity clears the pin
                if (elapsed >= _timeout) {
            ...
            case INDEXING_TO_NEXT: 
                conveyorOn(); // Turret outruns the next pin; belt stays on
            ...
            case MOVING_TO_DROP:
                conveyorOff(); // MUST halt. Turret is leaving the loading zone.
                if (_stepper.distanceToGo() == 0) { 
            ...
            case FETCH_10TH:
                ...
                if (_queuedPins > 0) {
                    _queuedPins--;
                    _loadedCount = 10;
                    conveyorOff(); // Halt belt immediately
                    tPrint("10th Pin Sensed! Falling through shaft...");
                    changeState(DWELLING_AT_DROP, 150); // Hardcoded 150ms gravity clear
                }
                break;
            ...
            case MOVING_TO_PURGE:
                conveyorOff();
                if (_stepper.distanceToGo() == 0) { changeState(DWELLING_AT_DROP, 1500); }
                break;
    ...
    bool commandExecuteDropSequence() {
        ...
        tPrint("Command: Execute Drop Sequence (Moving to Drop)");
        _dropComplete = false;        
        _isPurging = false;
    ...
    bool commandSuspendFetch() {
        if (_state == FETCH_10TH || _state == SLEEP_STARVED_FETCH || _state == MOVING_TO_DROP) {
...
private:
    ...
    unsigned long _stateStart, _timeout; int _loadedCount;
    long _targetPos;
...
public:
    enum State { IDLE, TWEENING };
    enum Pose { GUARD, UP, BACK, UNKNOWN };
    SweepController(Servo& left, Servo& right) : _left(left), _right(right), _state(IDLE), _currentPose(UNKNOWN) {}

    void begin() {
        _left.write(SWEEP_UP_ANGLE);
        _right.write(180 - SWEEP_UP_ANGLE);
    ...
    bool commandPose(Pose target) {
        ...
        _targetPose = target;
        _tweenStart = millis(); _state = TWEENING;
        return true;
    }
    bool isIdle() const { return _state == IDLE; }
    Pose getCurrentPose() const { return _currentPose; }
private:
    Servo& _left; Servo& _right; State _state;
    Pose _currentPose; Pose _targetPose;
...
public:
    ...
    void begin() {
        attachSlide();
        _scissors.write(SCISSOR_DROP_ANGLE);
    ...
    void update() {
        ...
        switch (_state) {
            case SETTLING_PINS_DOWN:
                if (elapsed >= PDROP_RAISE_SETTLE_MS) { _slide.write(SLIDER_RELEASE_ANGLE);
                    changeState(SETTLING_PINS_UP, PDROP_DROP_MS); }
                break;
            case SETTLING_PINS_UP:
                if (elapsed >= _timeout) { setRaise(RAISE_UP_ANGLE);
                    changeState(IDLE_UP, PDROP_RAISE_SETTLE_MS); }
                break;
            case GRABBING_DOWN:
                if (elapsed >= PDROP_RAISE_SETTLE_MS) { _scissors.write(SCISSOR_GRAB_ANGLE);
                    changeState(GRABBING_UP, 800); }
                break;
            case GRABBING_UP:
                if (elapsed >= _timeout) { setRaise(RAISE_UP_ANGLE);
                    changeState(IDLE_UP, PDROP_RAISE_SETTLE_MS); }
                break;
            case DROPPING_DOWN:
                if (elapsed >= PDROP_RAISE_SETTLE_MS) { _scissors.write(SCISSOR_DROP_ANGLE);
                    changeState(DROPPING_UP, 800); }
                break;
            case DROPPING_UP:
                if (elapsed >= _timeout) { setRaise(RAISE_UP_ANGLE);
                    changeState(IDLE_UP, PDROP_RAISE_SETTLE_MS); }
        ...
        if (_state == IDLE_UP && _timeout != 0) {
           if (elapsed >= _timeout) { _slide.write(SLIDER_HOME_ANGLE);
               _timeout = 0; }
        }
    }

    bool commandSetPins() { if (_state != IDLE_UP) return false;
        setRaise(RAISE_DOWN_ANGLE); changeState(SETTLING_PINS_DOWN); return true; }
    bool commandGrabPins() { if (_state != IDLE_UP) return false; setRaise(RAISE_GRAB_ANGLE); changeState(GRABBING_DOWN);
        return true; }
    ...
    void update() {
        ...
        switch (_state) {
            ...
            case WAITING_PIN_CLEAR:
                if (elapsed >= BALL_DOOR_WAIT_MS) { openDoor();
                    changeState(OPEN); }
                break;
            case OPEN:
...
public:
    ...
    bool updateAndCheck(bool systemReady) {
        ...
        if ((millis() - _speedDebounceTime) > 5) { 
        ...
        if ((millis() - _triggerDebounceTime) > 5) { 
...
public:
    ...
    void begin() {
        deckL.begin();
        deckR.begin(); laneL.begin(); laneR.begin();
...
private:
    ...
    void changeState(State s) {
        ...
        if (s == NORMAL) { setAll(255, 255, 255); showAll(); }     
        else if (s == PAUSED) { setAll(0, 0, 50); showAll(); }      
        else if (s == FILL) { setAll(0, 255, 0); showAll(); }       
    }
    ...
    void showAll() {
        deckL.show(); deckR.show();
        laneL.show(); laneR.show();
    }
};
...
public:
    ...
    void update() {
        ...
        switch (_state) {
            case BOOT_INIT:
                if (Sweep.isIdle() && Deck.isIdle()) { Sweep.commandPose(SweepController::GUARD);
                    Wled.triggerPreset(WLED_PRESET_BOOTING); changeState(BOOT_SWEEP_1_GUARD); DEBUG_PRINT("Boot: Step 1 - Initial Sweep Guard"); }
                break;
            case BOOT_SWEEP_1_GUARD:
                if (Sweep.isIdle()) { Sweep.commandPose(SweepController::BACK);
                    changeState(BOOT_SWEEP_1_BACK); DEBUG_PRINT("Boot: Step 2 - Sweep Back"); }
                break;
            case BOOT_SWEEP_1_BACK:
                if (Sweep.isIdle()) { Sweep.commandPose(SweepController::GUARD);
                    changeState(BOOT_SWEEP_1_GUARD_POST); }
                break;
            case BOOT_SWEEP_1_GUARD_POST:
                if (Sweep.isIdle()) { Deck.commandSetPins();
                    changeState(BOOT_DECK_CLEAR_1_WAIT); DEBUG_PRINT("Boot: Step 3 - Deck Clear 1"); }
                break;
            case BOOT_DECK_CLEAR_1_WAIT:
                if (Deck.isIdle()) { Sweep.commandPose(SweepController::BACK);
                    changeState(BOOT_SWEEP_2_BACK); DEBUG_PRINT("Boot: Step 4 - Sweep Clear 1"); }
                break;
            case BOOT_SWEEP_2_BACK:
                if (Sweep.isIdle()) { Sweep.commandPose(SweepController::GUARD);
                    changeState(BOOT_SWEEP_2_GUARD); }
                break;
            case BOOT_SWEEP_2_GUARD:
                if (Sweep.isIdle()) { changeState(BOOT_WAIT_TURRET_HOME);
                    DEBUG_PRINT("Boot: Step 5 - Waiting for Turret Home"); }
                break;
            case BOOT_WAIT_TURRET_HOME:
                if (Turret.isIdleEmpty()) { Turret.commandPurge();
                    changeState(BOOT_TURRET_PURGE_WAIT); DEBUG_PRINT("Boot: Step 6 - Turret Purge"); }
                break;
            case BOOT_TURRET_PURGE_WAIT:
                if (Turret.isDropComplete()) { changeState(BOOT_PURGE_DWELL); }
                break;
            case BOOT_PURGE_DWELL:
                if (millis() - _stateStart >= 1500) { Deck.commandSetPins();
                    changeState(BOOT_DECK_CLEAR_2_WAIT); DEBUG_PRINT("Boot: Step 7 - Deck Clear 2"); }
                break;
            case BOOT_DECK_CLEAR_2_WAIT:
                if (Deck.isIdle()) { Sweep.commandPose(SweepController::BACK);
                    changeState(BOOT_SWEEP_3_BACK); DEBUG_PRINT("Boot: Step 8 - Sweep Clear 2"); }
                break;
            case BOOT_SWEEP_3_BACK:
                if (Sweep.isIdle()) { Sweep.commandPose(SweepController::GUARD);
                    changeState(BOOT_SWEEP_3_GUARD); }
                break;
            case BOOT_SWEEP_3_GUARD:
                if (Sweep.isIdle()) { changeState(BOOT_WAIT_TURRET_IDLE_1);
                    DEBUG_PRINT("Boot: Step 9 - Waiting for Turret to finish Homing"); }
                break;
            ...
            case BOOT_WAIT_TURRET_9:
                if (Turret.isPausedAt9()) { Turret.commandExecuteDropSequence();
                    changeState(BOOT_TURRET_PRIME_DROP); DEBUG_PRINT("Boot: Dropping 10 into empty Deck."); }
                break;
            ...
            case T1_DECK_GRAB:
                if (Deck.isIdle()) { Sweep.commandPose(SweepController::BACK);
                    changeState(T1_SWEEP_BACK); }
                break;
            case T1_SWEEP_BACK:
                if (Sweep.isIdle()) { Sweep.commandPose(SweepController::GUARD);
                    changeState(T1_SWEEP_GUARD_POST); }
                break;
            case T1_SWEEP_GUARD_POST:
                if (Sweep.isIdle()) { Deck.commandDropPins();
                    changeState(T1_DECK_DROP); }
                break;
            case T1_DECK_DROP:
                if (Deck.isIdle()) { Sweep.commandPose(SweepController::UP);
                    changeState(T1_SWEEP_UP); }
                break;
            case T1_SWEEP_UP:
            ...
            case T2_PIN_SETTLE_WAIT:
                if (millis() - _stateStart >= 3000) { Sweep.commandPose(SweepController::BACK);
                    changeState(T2_SWEEP_BACK); }
                break;
            case T2_SWEEP_BACK:
                if (Sweep.isIdle()) { Sweep.commandPose(SweepController::GUARD);
                    changeState(RESET_SWEEP_GUARD_POST); }
                break;
            ...
            case RESET_WAIT_TURRET_IDLE:
                if (_backgroundRestartPending) {
                    Door.triggerCycle();
                    _throwCount = 1; changeState(WAITING_FOR_BALL); 
                }
                else if (Turret.isIdleEmpty()) {
                    Turret.commandStartRefill();
                    Door.triggerCycle(); _throwCount = 1; clearFillBall(); changeState(WAITING_FOR_BALL); 
                }
                else if (!Turret.isHoming() && !Turret.isCommittedToDrop()) {
                    Turret.commandWake();
            ...
            case RESET_BUFFER_DWELL:
                if (millis() - _stateStart >= 1500) { Deck.commandSetPins();
                    _deckHasPins = false; changeState(RESET_DECK_SET); }
                break;
            case MANUAL_RESET_START:
                if (Sweep.isIdle()) { Sweep.commandPose(SweepController::BACK);
                    changeState(MANUAL_RESET_BACK); }
                break;
    ...
    void clearFillBall() { _fillBallArmed = false; }
    ...
    void exitMaintenanceMode() {
        ...
        _deckHasPins = false; _backgroundDropActive = false;
        _backgroundRestartPending = false;
...
void loop() {
    ...
    if (btnEvent == ButtonController::SHORT_PRESS) {
        if (Game.isMaintenanceActive()) {
            ...
            Game.triggerLaneReset();
            ScoreMoreSerial.sendResetPulse(); 
        }
